// Scripts/options/option_11_5.js
module.exports = async function (helpers) {
  ...
  const containerName = 'humrine-web-staging';
  const backupDir = '/app/data/backups';   // inside the container

  try {
    // List .json files safely – exits 0 even if none found
    const listCmd = `ssh -q -i "${sshKey}" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${sshUser}@${vmIp} "sudo docker exec ${containerName} sh -c 'ls -1 ${backupDir}/*.json 2>/dev/null || true'"`;

    console.log('📂 Fetching backup file list...');
    const output = execSync(listCmd, { encoding: 'utf-8', stdio: 'pipe' }).trim();

    if (!output) {
      console.log(`\x1b[33m⚠ No backup files found in ${backupDir} inside ${containerName}.\x1b[0m`);
      await pause();
      return;
    }
    ...
    const restoreCmd = `ssh -q -i "${sshKey}" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${sshUser}@${vmIp} "sudo docker exec ${containerName} /app/humrine_site_linux restore_db ${selectedFile} --yes"`;
